...
| # | 功能 | 说明 | 现状 |
|---|---|---|---|
| R1.6-1 | **订单详情/履约跟踪页** ⚠️ 最高优先 | 目前只有订单列表，无详情页：单证清单与齐备度、双边报关状态、航段时刻、冷链温度曲线、资金与账本流水**全部看不到**。后端读接口已就绪（`GET /orders/:code/shipment`、`/orders/:code/doc-checklist`、`/documents/received`、`/finance/ledger`） | **R1.5-8 引导文案已向用户承诺此页，属欠账，须优先补齐** |
| R1.6-2 | 供应商自助录入补齐 | 联系人、资质证书、CITES 配额、产品 attributes（营养/品鉴）目前只能脚本或 API 写入 —— 三家真实供应商均由运营代录，规模化瓶颈 | 后端 API 多数已有，缺工作台界面 |
| R1.6-3 | 单证上传与预览 | 单证仅登记元数据；原件 PDF 需 S3（R1-3）落地后开放上传与在线预览、脱敏副本像素级遮盖 | 依赖 R1-3 |
| R1.6-4 | 订单时间线 | 状态机流转历史（谁在何时把订单推到哪个状态）已有审计数据，缺可视化 | 数据已备 |
